...
export default function NeuralNetworkDiagram({
  ...
  const { layers, connections, layerSpacing } = useMemo(() => {
    ...
    const generatedConnections: { x1: number; y1: number; x2: number; y2: number; key: string; length: number }[] = [];
    ...
    return { layers: generatedLayers, connections: generatedConnections, layerSpacing: spacing };
  }, [nodeCount]);
