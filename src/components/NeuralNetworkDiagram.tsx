...
export default function NeuralNetworkDiagram({
  ...
  // ⚡ Bolt: Encapsulate layers calculation in useMemo to prevent unnecessary re-evaluations
  // Performance Impact: Avoids reconstructing the entire node grid on re-renders when nodeCount hasn't changed.
  ...
  // ⚡ Bolt: Calculate connection lengths mathematically during render instead of querying DOM in useEffect.
  // Performance Impact: Eliminates forced synchronous layout (layout thrashing) on component mount, significantly improving rendering time.
      ...
      >
        ...
        {connections.map((conn) => (
          <line
            ...
            strokeWidth="1"
            opacity="0.3"
            // ⚡ Bolt: Pass calculated path length to CSS variables instead of imperative DOM manipulation
            style={{
