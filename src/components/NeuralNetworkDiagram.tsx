...
export default function NeuralNetworkDiagram({
  ...
  const { width, height, layerSpacing, layers, connections } = useMemo(() => {
    ...
    const lay = nodeCount.map((count, layerIndex) => {
      const nodes = [];
    ...
    const conn: { x1: number; y1: number; x2: number; y2: number; key: string; length: number; index: number }[] = [];
    let connIndex = 0;
    for (let i = 0; i < lay.length - 1; i++) {
      const currentLayer = lay[i];
      const nextLayer = lay[i + 1];
    ...
    return { width: w, height: h, layerSpacing: spacing, layers: lay, connections: conn };
  }, [nodeCount]);
