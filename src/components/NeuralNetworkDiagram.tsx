...
export default function NeuralNetworkDiagram({
  ...
  // ⚡ Bolt Optimization: Precalculate layout and connection lengths during render phase
  // Eliminates imperative DOM `.querySelectorAll` and `.getAttribute` queries inside `useEffect`
  // which previously caused layout thrashing and prevented static rendering/SSR.
  const { layers, layerSpacing, connections } = useMemo(() => {
    const spacing = width / (nodeCount.length + 1);
    ...
    const calcConnections: { x1: number; y1: number; x2: number; y2: number; key: string; length: number; index: number }[] = [];
    let connIndex = 0;
    ...
    return { layers: calculatedLayers, layerSpacing: spacing, connections: calcConnections };
  }, [nodeCount]);
      ...
      >
        <defs>
          ...
          <style>
            {`
              @keyframes lineFlow {
                0% { stroke-dashoffset: var(--path-length); opacity: 0.2; }
                50% { opacity: 0.6; }
