...
export default function NeuralNetworkDiagram({
  ...
  const { layers, connections, layerSpacing } = useMemo(() => {
    ...
    const l = nodeCount.map((count, layerIndex) => {
      const nodes = [];
    ...
    const conn: { x1: number; y1: number; x2: number; y2: number; key: string; length: number; delay: number }[] = [];
    for (let i = 0; i < l.length - 1; i++) {
      const currentLayer = l[i];
      const nextLayer = l[i + 1];
    ...
    return { layers: l, connections: conn, layerSpacing: lSpacing };
  }, [nodeCount]);
        ...
        {connections.map((conn) => (
          <line
            ...
            style={{
              ...
              animation: animated ? `lineFlow 3s ${conn.delay}s ease-in-out infinite` : undefined
            } as CSSProperties}
          />
        ))}
