...
export default function NeuralNetworkDiagram({
  ...
  const { layerSpacing, layers, connections } = useMemo(() => {
    ...
    const newLayers = nodeCount.map((count, layerIndex) => {
      const nodes = [];
    ...
    const newConnections: { x1: number; y1: number; x2: number; y2: number; key: string; length: number; index: number }[] = [];
    ...
    return { layerSpacing: spacing, layers: newLayers, connections: newConnections };
  }, [nodeCount]);
        ...
        <defs>
          ...
          <style>
            {`
              @keyframes lineFlow {
                /*
                 * Use CSS custom property (var(--path-length)) to dynamically assign stroke-dashoffset based on Math precalculations in useMemo
                 * By removing imperatively reading element.getAttribute inside useEffect, we bypass synchronous layout thrashing during TTI
                 */
