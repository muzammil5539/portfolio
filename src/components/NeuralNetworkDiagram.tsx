...
export default function NeuralNetworkDiagram({
  ...
  const { layers, connections, width, height, layerSpacing } = useMemo(() => {
    const w = 600;
    const h = 400;
    const spacing = w / (nodeCount.length + 1);
    ...
    const parsedLayers = nodeCount.map((count, layerIndex) => {
      const nodes = [];
    ...
    const parsedConnections: { x1: number; y1: number; x2: number; y2: number; key: string; length: number; index: number }[] = [];
    let connIndex = 0;
    for (let i = 0; i < parsedLayers.length - 1; i++) {
      const currentLayer = parsedLayers[i];
      const nextLayer = parsedLayers[i + 1];
    ...
    return { layers: parsedLayers, connections: parsedConnections, width: w, height: h, layerSpacing: spacing };
  }, [nodeCount]);
