...
export default function NeuralNetworkDiagram({
  ...
  const { layerSpacing, layers, connections } = useMemo(() => {
    const computedLayerSpacing = width / (nodeCount.length + 1);

    const getNodePosition = (layerIndex: number, nodeIndex: number, totalNodes: number) => {
      const x = computedLayerSpacing * (layerIndex + 1);
    ...
    return { layerSpacing: computedLayerSpacing, layers: computedLayers, connections: computedConnections };
  }, [nodeCount]);
