...
export default function NeuralNetworkDiagram({
  ...
  // Performance optimization: Pre-calculate layers, connections, and lengths in useMemo
  // to avoid querying the DOM inside useEffect, which caused layout thrashing and blocked SSR.
  const { layers, connections, layerSpacing } = useMemo(() => {
    const lSpacing = width / (nodeCount.length + 1);
    ...
    return { layers: newLayers, connections: newConnections, layerSpacing: lSpacing };
  }, [nodeCount]);
