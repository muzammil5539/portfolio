...
export default function NeuralNetworkDiagram({
  ...
  const { layerSpacing, layers, connections } = useMemo(() => {
    ...
    const connections: { x1: number; y1: number; x2: number; y2: number; key: string; length: number }[] = [];
    ...
    return { layerSpacing, layers, connections };
  }, [nodeCount]);
        ...
        {connections.map((conn, i) => (
          <line
            ...
            style={{
              ...
              animation: animated ? `lineFlow 3s ${i * 0.05}s ease-in-out infinite` : undefined
            } as CSSProperties}
          />
        ))}
