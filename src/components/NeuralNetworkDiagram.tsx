...
export default function NeuralNetworkDiagram({
  ...
  // ⚡ Bolt Performance: Calculate lengths mathematically during render and useMemo
  // This prevents layout thrashing via DOM queries and enables SSR/static rendering
  const { layers, connections, layerSpacing } = useMemo(() => {
    ...
    const computedConnections: { x1: number; y1: number; x2: number; y2: number; length: number; key: string }[] = [];
    ...
    return { layers: computedLayers, connections: computedConnections, layerSpacing: spacing };
  }, [nodeCount]);
