...
export default function NeuralNetworkDiagram({
  ...
  const { width, height, layerSpacing, layers, connections } = useMemo(() => {
    // ⚡ Bolt Performance Optimization:
    // We pre-calculate all node positions and SVG connection lengths mathematically
    // in this useMemo block rather than relying on DOM measurements (querySelectorAll + getAttribute)
    // inside a useEffect hook. This eliminates layout thrashing/Forced Synchronous Layout
    // on component mount and makes the SVG fully compatible with Next.js SSR.
    // Impact: Removes a synchronous rendering block during hydration and improves Time to Interactive (TTI).

    const w = 600;
    const h = 400;
    const lSpacing = w / (nodeCount.length + 1);

    const getNodePosition = (layerIndex: number, nodeIndex: number, totalNodes: number) => {
      const x = lSpacing * (layerIndex + 1);
      const nodeSpacing = h / (totalNodes + 1);
      ...
      if (currentLayer && nextLayer) {
        currentLayer.forEach((node, ni) => {
          nextLayer.forEach((nextNode, nj) => {
            ...
            // Mathematically pre-calculate the hypotenuse to avoid needing getTotalLength() from the DOM
    ...
    return { width: w, height: h, layerSpacing: lSpacing, layers: computedLayers, connections: computedConnections };
  }, [nodeCount]);
