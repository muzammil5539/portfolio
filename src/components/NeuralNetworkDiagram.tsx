...
export default function NeuralNetworkDiagram({
  ...
  // ⚡ Bolt: Moving heavy layout calculations into useMemo avoids re-running math on every render
  // and eliminates imperative DOM queries inside useEffect (which cause layout thrashing and SSR errors).
  const { layers, connections, layerSpacing } = useMemo(() => {
    ...
    const connections: { x1: number; y1: number; x2: number; y2: number; length: number; key: string }[] = [];
    ...
    return { layers, connections, layerSpacing };
  }, [nodeCount]);
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      >
        ...
        {/* Layer labels */}
        {LAYER_LABELS.slice(0, nodeCount.length).map((label, i) => (
          <text
            key={`label-${i}`}
            x={layerSpacing * (i + 1)}
            y={height - 20}
