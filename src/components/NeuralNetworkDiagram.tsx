...
export default function NeuralNetworkDiagram({
  ...
  const connections = useMemo(() => {
    const conns: { x1: number; y1: number; x2: number; y2: number; key: string; length: number; globalIndex: number }[] = [];
    let globalIndex = 0;
        ...
        {connections.map((conn) => {
          const style: CSSProperties = animated
            ? {
                ...
                animation: `lineFlow 3s ${conn.globalIndex * 0.05}s ease-in-out infinite`,
              }
            : {
