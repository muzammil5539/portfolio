...
export default function NeuralNetworkDiagram({
  ...
  // Performance optimization: Pre-calculate the computationally expensive layer nodes and connection lengths
  ...
  const { layers, connections, layerSpacing } = useMemo(() => {
    ...
    const calculatedConnections: { x1: number; y1: number; x2: number; y2: number; key: string; length: number }[] = [];
    ...
    return { layers: calculatedLayers, connections: calculatedConnections, layerSpacing: spacing };
  }, [nodeCount]);
