...
export default function NeuralNetworkDiagram({
  ...
  // Precalculate coordinate distances mathematically during the render phase (via useMemo)
  // to avoid layout thrashing that would occur if we used DOM queries in useEffect.
  const { layers, connections } = useMemo(() => {
    ...
    const generatedLayers = nodeCount.map((count, layerIndex) => {
      const nodes = [];
    ...
    const generatedConnections: { x1: number; y1: number; x2: number; y2: number; length: number; key: string }[] = [];
    for (let i = 0; i < generatedLayers.length - 1; i++) {
      const currentLayer = generatedLayers[i];
      const nextLayer = generatedLayers[i + 1];
    ...
    return { layers: generatedLayers, connections: generatedConnections };
        ...
        {connections.map((conn, index) => (
          <line
            ...
            style={
              animated
                ? ({
                    animation: `lineFlow 3s ${index * 0.05}s ease-in-out infinite`,
                  } as CSSProperties)
                : undefined
            }
          />
