...
export default function NeuralNetworkDiagram({
  ...
  const { layers, connections, layerSpacing } = useMemo(() => {
    ...
    // Performance optimization: Precalculate connection lengths mathematically during render phase
    // instead of using imperative DOM queries (querySelectorAll + getAttribute) inside useEffect.
    // This prevents layout thrashing, avoids double-renders, and enables proper SSR.
    ...
    return { layers: calculatedLayers, connections: calculatedConnections, layerSpacing };
  }, [nodeCount]);
