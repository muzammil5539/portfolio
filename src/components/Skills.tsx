    ...
    >
      ...
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
          ...
          {Object.entries(skillsData).map(([category, data]) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-6">
                <div className={`w-10 h-10 rounded-lg bg-ai-navy flex items-center justify-center text-${data.color} group-hover:shadow-glow-cyan transition-shadow`}>
                  {data.icon}
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {data.skills.map((skill, index) => (
                  <span
                    key={index}
                    className={`px-3 py-1.5 bg-ai-navy/80 text-ai-text-muted text-sm font-medium rounded-lg border border-ai-slate/50 hover:border-${data.color}/50 hover:text-ai-text transition-all duration-200`}
                  >
                    {skill}
                  </span>
                ))}
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            { value: "15+", label: "Technologies", color: "ai-cyan" },
            { value: "6+", label: "Projects", color: "ai-blue" },
            { value: "4+", label: "Domains", color: "ai-purple" },
            { value: "1+", label: "Years Experience", color: "ai-green" },
          ].map((stat, index) => (
